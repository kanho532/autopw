...
所有探索式测试计划、截图、trace、日志和报告必须写入被测试项目根目录下的唯一审查目录：

```text
<测试项目根目录>/autopw-output/<运行模式>-<UTC时间>/
```

其中 `<运行模式>` 使用 `FULL`、`COMMIT_TO_WORKTREE` 或 `COMMIT_TO_COMMIT`，`<UTC时间>` 使用 `YYYYMMDDTHHmmssZ`。将该目录记为 `<audit-root>`，不得把产物写入插件源码目录或复用旧审查目录。

## 输入与安全

确定目标 URL、范围、凭据和允许的数据变更；输出目录固定为前述 `<audit-root>`。在开始时记录 ISO 8601 时间，并为每个页面流程或计划用例记录开始、结束与 `duration_ms`。
...
读取 [report-template.md](assets/report-template.md)，将其完整结构复制到 `<audit-root>/report.md`。该模板为强制要求：保留其中文标题、章节顺序、问题字段、问题汇总表、覆盖范围小节和备注。按严重级别排列已验证问题，并列出已测试、未测试、阻塞及有意跳过的破坏性流程。

使用标准 Markdown 图片链接和相对制品路径。不要使用专有媒体标记语法。

分别记录确切的浏览器执行器、测试运行器和回退通道。为每个计划用例回填 `PASS`、`FAIL`、`BLOCKED`、`NOT_RUN` 或 `FLAKY`；根据这些状态而不是问题数量计算用例总数。

使用模板的“执行时间”章节记录每个通道、用例和尝试的开始、结束与耗时；重试不得覆盖首次记录。

...
Memory 缺失或失效时读取完整 preflight；Memory 命中时仍复核命令、Jar 和目标端口，并独立检查 AutoPW bundled runtime 与 Chromium。缺少 Memory、Schema validation 失败、executor/lifecycle 异常或 Playwright resolution 异常时，才读取详细 runtime reference。

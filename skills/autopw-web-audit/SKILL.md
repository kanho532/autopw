...
除非用户选择其他位置，否则将计划写入 `autopw-output/test-plan.md`。生成或执行测试前完成该计划。执行开始后，不要修改原用例的范围或断言；每个完整业务流结束后立即回填状态和证据，并将新发现的覆盖项以 `DISCOVERED-<n>` 追加为后续用例，不要重写原计划。

### 3. 预检、分组并启动

阅读 [execution-and-evidence.md](references/execution-and-evidence.md)，在判定阻塞前遵循其中的发现顺序。

1. 优先发现仓库本地包装器、已有 Playwright Test 配置、测试依赖和文档中的启动命令。对于步骤和断言已经明确的 `BROWSER` 用例，将相关用例合并到本次运行专属的 `.spec.*` 中，并通过一次测试运行器调用批量执行。Playwright MCP 是浏览器执行器，不能证明 Playwright Test 已运行。
2. 发现配置为 `autopw-playwright` 的插件内置 MCP 服务器。仅将其用于页面结构未知的交互探索、批量测试失败后的复现与诊断，或浏览器与直接 API 行为不一致的情况。首次使用前执行一次无害的导航/快照预检；同一审查中的相关流程复用页面和浏览器上下文，只有隔离要求存在时才新建上下文。不要猜测宿主特定的工具命名空间；根据服务器标识和能力识别工具。
...
当请求包含创建可复用测试时，生成有针对性的 Playwright spec。执行审查时，使用现有测试或本次运行专属的临时 Playwright 工具，不要修改目标项目依赖。步骤和断言明确的浏览器用例应按相关业务流批量生成并通过一次测试运行器调用执行，同时保留各用例独立的断言、状态和制品。每个工作器只执行分配给它的用例 ID，并将证据和状态写入对应通道目录。
...
使用 Playwright MCP 时，从一个已知的隔离页面开始，在完整业务流的关键检查点收集可访问性/DOM 快照、控制台消息、网络请求、失败请求、URL 和截图；如果操作没有产生需要判断的新状态，不要在每次点击或输入后重复采集。仅使用 MCP 工具时，不得声称事件监听器或 Playwright Test 已运行。
...
每次有意义的变更后，在可以访问时重新读取权威 API 或持久化状态。使用 Playwright trace 和截图证明浏览器问题，但仅在它们能提供实质性证据时使用。不要在制品中保留凭据和敏感响应体。只在声明的依赖屏障处同步；失败用例只阻塞依赖它的用例，不能阻塞无关通道。每个完整业务流结束后立即更新冻结计划中的状态和证据；运行时新发现只追加为 `DISCOVERED-<n>`，不得改变原用例的范围或断言。
...
- 批量执行步骤和断言明确的浏览器用例；只用 Playwright MCP 探索未知页面状态、复现失败或诊断浏览器差异。
